#!/usr/bin/env python3
"""
Test script for the categorize-tool-usecase endpoint.
This script takes a tool ID as input and hits the API endpoint to show use case categorization results.
"""
...
USECASE_ENDPOINT = "/categorize-tool-usecase"
...
def test_categorize_tool_usecase(tool_id):
    """Test the categorize-tool-usecase endpoint with the given tool ID."""
    
    print_header(f"TESTING CATEGORIZE TOOL USE CASE ENDPOINT")
    print(f"🎯 Tool ID: {tool_id}")
    print(f"🌐 API URL: {API_BASE_URL}")
    print(f"📅 Test Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    # Construct the full URL
    url = f"{API_BASE_URL}{USECASE_ENDPOINT}/{tool_id}"
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                print_subheader("USE CASE CATEGORIZATION RESULTS")
                
                # Display tool information
                tool_name = response_data.get('tool_name', 'Unknown')
                matched_usecases = response_data.get('matched_usecases', [])
                total_available = response_data.get('total_available_usecases', 0)
                
                print(f"🔧 Tool Name: {tool_name}")
                print(f"📋 Total Available Use Cases: {total_available}")
                print(f"🎯 Matched Use Cases: {len(matched_usecases)}")
                
                if matched_usecases:
                    print("\n📂 SELECTED USE CASES:")
                    for i, usecase in enumerate(matched_usecases, 1):
                        uc_id = usecase.get('id', 'N/A')
                        uc_name = usecase.get('name', 'N/A')
                        print(f"  {i:2d}. {uc_name}")
                        print(f"      ID: {uc_id}")
                        
                        # Add separator for readability
                        if i < len(matched_usecases):
                            print(f"      {'-'*40}")
                else:
                    print("\n⚠️  No use cases were matched for this tool.")
...
def get_tool_id():
    ...
    print_header("CATEGORIZE TOOL USE CASE TESTER")
    print("This script will test the categorize-tool-usecase endpoint")
    print(f"API Server: {API_BASE_URL}")
    
    while True:
        tool_id = input("\n🔧 Enter Tool ID to categorize use cases for: ").strip()
...
def main():
    ...
    try:
        ...
        # Run the test
        test_categorize_tool_usecase(tool_id)
...
if __name__ == "__main__":
    print("🚀 Starting Use Case Tool Test Script...")
    main()
